define(['jquery', 'bootstrap', 'layer', 'jquery.dragsort'], function($, b, layer, drag){
    var options = {
        ...
        uploadState:'pending',
        layerIndex:0,
        uploadServerUrl:'/phpapi/upload.php',
        sortServerUrl:'/phpapi/resort.php',
        delServerUrl:'/phpapi/del.php',
        fileQueueList:[],
        thumb:{
            width:200,
            height:100,
        ...
        fileUploadedItemTpl:'<dd data-file-id="{{id}}" data-index="{{index}}" data-sort="{{sort}}" title="拖动改变排序">'+
                                ...
                                '<img src="{{url}}" class="file-img-thumb">'+
        ...
        require(['webuploader'], function(WebUploader){
            ...
            uploader.on('uploadSuccess', function(file, ret){
                ...
                if (ret.code && ret.code == 200) {
                    ...
                    html = options.fileUploadedItemTpl.replace('{{id}}', ret.data.id).replace('{{url}}', ret.data.url).replace('{{sort}}', ret.data.sort).replace('{{index}}', length);
                    $(html).appendTo(options.uploadedFileList);
                    $li.remove();
                } else {
