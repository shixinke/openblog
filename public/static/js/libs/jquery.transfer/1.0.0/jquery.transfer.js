...
(function($){
    ...
    $.fn.transfer = function(options) {
        ...
        return this.each(function(){
            ...
            for (var i=0; i< transfer.targetItemList.length; i++) {
                targetHtml += settings.itemTpl.replace(/\{\{value\}\}/g, transfer.targetItemList[i].value).replace(/\{\{text\}\}/g, transfer.targetItemList[i].text).replace('{{name}}', transfer.options.formName);
                initValues.push(transfer.targetItemList[i].value);
            }
            ...
            $html.find('.transfer-hidden-value').val(initValues.join(','));
    ...
    $.fn.transfer.defaults = {
        'template':                                         
            '<div class="transfer">\
                 <input type="hidden" class="transfer-hidden-value" id="transfer-hidden" name="{{name}}">\
        ...
        'itemTpl':'<label class="transfer-panel-item" data-text="{{text}}" data-value="{{value}}">\
                       <span class="checkbox-input pull-left">\
                           <input type="checkbox" class="" value="{{value}}">\
                       </span>\
        ...
        'iconLeft':'icon-small-left',
        'iconRight':'icon-small-right'
    }
})(jQuery);